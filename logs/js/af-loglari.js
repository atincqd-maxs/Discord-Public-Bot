...
async function loadModeratorFilterData() {
    try {
        const res = await fetch(`../api.php?action=get_af_log_moderators`);
        ...
        console.error('Affeden Yetkili filtresi yüklenemedi:', e);
        showToast('Affeden Yetkili filtresi verileri yüklenemedi.');
    }
}
...
async function loadLogs(page = 1) {
    ...
    try {
        const apiUrl = `../api.php?action=get_af_logs&page=${page}&sort=${sort}&moderator_id=${moderator_id}`;
        ...
        if (data.logs.length === 0) {
            logList.innerHTML = '<p style="text-align:center; color:var(--text-muted);">Bu filtrelerle eşleşen af logu bulunamadı.</p>';
        } else {
            data.logs.forEach(log => {
                const card = document.createElement('div');
                card.className = 'ceza-log-card'; // ceza-log-card stilini kullanalım
                ...
                card.innerHTML = `
                    <div class="ceza-header">
                        ...
                        <span class="ceza-id" style="margin-left: 10px;">Af ID: #${log.id} (Ceza ID: #${log.original_ceza_id})</span>
                        <span class="ceza-status status-bitti" style="margin-left: auto;">Affedildi</span>
                    </div>
                    <div class="ceza-users">
                        <div class="user-info">
                            ...
                            <span class="user-role-label">Affeden Yetkili</span>
                        </div>
                    ...
                    <div class="ceza-details">
                        <div class="detail-item">
                            <span class="detail-label">Affedilen Cezanın Sebebi:</span>
                            <span class="detail-value reason">${log.original_sebep || 'Belirtilmemiş'}</span>
                        </div>
                    </div>
                     <div class="ceza-timing">
                        <div class="time-item">
                            <span class="time-label">Affedilme Zamanı:</span>
                            <span class="time-value">${formatTimestamp(log.forgive_timestamp)}</span>
                        </div>
                    </div>
                `;
...
function renderPagination(pagination) {
    ...
    const maxVisibleButtons = 5; // Ortadaki sayfa sayısı + sağ/sol oklar
    const sideButtons = Math.floor(maxVisibleButtons / 2);

    const prevButton = document.createElement('button');
    prevButton.innerHTML = '&laquo;'; // Sol ok
    ...
    // Kenarlara yaslanma durumlarını ayarla
    ...
    // Ortadaki sayfa düğmeleri
    for (let i = startPage; i <= endPage; i++) {
        const pageButton = document.createElement('button');
        pageButton.textContent = i;
        if (i === current_page) {
            pageButton.disabled = true;
            pageButton.classList.add('active'); // Aktif sayfa stili için
        }
    ...
    nextButton.innerHTML = '&raquo;'; // Sağ ok
