...
async function loadFilterData() {
    try {
        const res = await fetch(`../api.php?action=get_log_users`);
...
function translateStatus(status) {
    ...
    switch (failReason) {
        ...
        case 'INVALID_ARGS': statusText = 'Geçersiz Argüman'; break;
        case 'INVALID_DURATION_FORMAT': statusText = 'Geçersiz Süre'; break;
        case 'DURATION_TOO_LONG': statusText = 'Süre Çok Uzun'; break;
        case 'DURATION_TOO_SHORT': statusText = 'Süre Çok Kısa'; break;
        case 'NO_REASON': statusText = 'Sebep Gerekli'; break;
        case 'NO_JAIL_ROLE_CONFIGURED': statusText = 'Jail Rolü Ayarlı Değil'; break;
        case 'JAIL_ROLE_NOT_FOUND': statusText = 'Jail Rolü Bulunamadı'; break;
        case 'NO_PERMISSION (HIERARCHY)': statusText = 'Yetki Yok (Hiyerarşi)'; break;
...
async function loadLogs(page = 1) {
    ...
    try {
        const res = await fetch(`../api.php?action=get_logs&page=${page}&sort=${sort}&executor_id=${executor_id}`);
        const data = await res.json();
