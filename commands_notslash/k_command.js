...
module.exports = {
  ...
  async execute(message, args, db, settings) {
    ...
    if (!targetId || !nameInput || !age) {
      await logToDb(message.client, settings, db, 'k', authorMember, null, 'FAIL: INVALID_ARGS');
      return;
    }

    const ageNum = parseInt(age);
    if (isNaN(ageNum) || ageNum < 10 || ageNum > 60) {
      await logToDb(message.client, settings, db, 'k', authorMember, { id: targetId }, `FAIL: INVALID_AGE (${ageNum})`);
      return;
    }

    const nameRegex = /^[a-zA-ZğüşıöçİĞÜŞÖÇ]+$/;
    if (!nameRegex.test(nameInput)) {
      await logToDb(message.client, settings, db, 'k', authorMember, { id: targetId }, 'FAIL: INVALID_NAME');
      return;
    }
    ...
    try {
      ...
      await logToDb(message.client, settings, db, 'k', authorMember, { id: targetId, user: { tag: 'Bilinmiyor' } }, 'FAIL: TARGET_NOT_FOUND');
      try {
    ...
    collector.on('collect', async (interaction) => {
      ...
      try {
        ...
        if (roleIdToRemove) {
            try {
                ...
                await logToDb(message.client, settings, db, 'k', authorMember, targetMember, `FAIL: OLD_ROLE_REMOVE_ERROR (${roleRemoveError.message})`);
            }
        }
        ...
        if (roleIdToAdd) {
            try {
                ...
                if (role) {
                    ...
                    await logToDb(message.client, settings, db, 'k', authorMember, targetMember, `FAIL: CONFIGURED_ROLE_NOT_FOUND ID: ${roleIdToAdd}`);
                }
            } catch (roleError) {
                console.error("Kayıt rolü verme hatası:", roleError);
                await logToDb(message.client, settings, db, 'k', authorMember, targetMember, `FAIL: ROLE_ADD_ERROR (${roleError.message})`);
            }
        }
        ...
        if (err.code === 50013) {
           await logToDb(message.client, settings, db, 'k', authorMember, targetMember, 'FAIL: NO_PERMISSION (HIERARCHY)');
           try {
          ...
          console.error(`'.k' komutu (nickname/role) başarısız oldu:`, err);
          await logToDb(message.client, settings, db, 'k', authorMember, targetMember, `FAIL: ${err.message}`);
        }
        collector.stop();
        return;
      }
      ...
      try {
        ...
        await logToDb(message.client, settings, db, 'k', authorMember, targetMember, `FAIL: DB_INSERT_ERROR ${dbError.message}`);
        try {
      ...
      await logToDb(message.client, settings, db, 'k', authorMember, targetMember, `SUCCESS: ${newNickname} (${gender})`);
