...
module.exports = {
  ...
  async execute(message, args, db, settings) {
    ...
    const uyari1Roles = otherGuildConfig.uyari1_roles || {};
    ...
    if (!uyari2RoleId) {
        console.error(`[${commandName}] Hata: Uyarı 2 rolü ayarlanmamış. Sunucu: ${message.guild.id}, Kullanan: ${authorMember.id}, Hedef: ${targetId}`);
        await logToDb(message.client, settings, db, commandName, authorMember, targetMember, 'FAIL: NO_UYARI2_ROLE_CONFIGURED');
        message.react(emojis.ERROR).catch(e => { if (e.code !== 10008) console.error("Hata tepkisi eklenemedi:", e); });
        return;
    }

    if (uyari1RoleId === uyari2RoleId && uyari2RoleId !== null) {
    ...
    let uyariRole;
    try {
        uyariRole = await message.guild.roles.fetch(uyari2RoleId);
        if (!uyariRole) throw new Error('Rol bulunamadı');
    } catch (roleError) {
         console.error(`[${commandName}] Hata: Ayarlanan uyarı 2 rolü (${uyari2RoleId}) sunucuda bulunamadı. Sunucu: ${message.guild.id}, Kullanan: ${authorMember.id}, Hedef: ${targetId}`);
         await logToDb(message.client, settings, db, commandName, authorMember, targetMember, 'FAIL: UYARI2_ROLE_NOT_FOUND');
         message.react(emojis.ERROR).catch(e => { if (e.code !== 10008) console.error("Hata tepkisi eklenemedi:", e); });
         return;
    }
    ...
    if (botMember.roles.highest.position <= uyariRole.position) {
        console.error(`[${commandName}] Hata: Bot hiyerarşisi uyarı 2 rolü için yetersiz. Bot Rolü Pozisyon: ${botMember.roles.highest.position}, Uyarı Rolü Pozisyon: ${uyariRole.position}. Kullanan: ${authorMember.id}, Hedef: ${targetId}`);
    ...
    try {
         const hasUyari1 = uyari1RoleId ? targetMember.roles.cache.has(uyari1RoleId) : false;
         const hasUyari2 = targetMember.roles.cache.has(uyari2RoleId);

        if (hasUyari2) {
             console.error(`[${commandName}] Hata: Kullanıcı (${targetId}) zaten uyarı 2 rolüne sahip. Kullanan: ${authorMember.id}`);
        ...
        if (hasUyari1) {
             try {
                ...
                const uyari1RoleToRemove = await message.guild.roles.fetch(uyari1RoleId);
                 if (uyari1RoleToRemove && botMember.roles.highest.position > uyari1RoleToRemove.position) {
                    await targetMember.roles.remove(uyari1RoleToRemove, `Uyarı 2 veriliyor: Yetkili ${authorMember.user.tag}`);
                } else {
                ...
                console.error(`[${commandName}] Uyarı 1 rolü (${uyari1RoleId}) kaldırılamadı:`, removeError);
                await logToDb(message.client, settings, db, commandName, authorMember, targetMember, `FAIL: REMOVE_UYARI1_ROLE_ERROR (${removeError.message})`);
            }
        }
        ...
        await targetMember.roles.add(uyariRole, `Uyarı 2: Yetkili ${authorMember.user.tag} - Sebep: ${reason}`);
