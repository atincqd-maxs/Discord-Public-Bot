...
module.exports = {
  name: 'unmute',
  description: 'Bir kullanıcının aktif chat susturmalarını kaldırır.',
  async execute(message, args, db, settings) {
    const authorMember = message.member;
    const targetId = args[0];
    const cezaTipi = 'chatmute';
    ...
    try {
      ...
      if (activePunishments.length === 0) {
        console.error(`[${commandName}] Hata: Aktif chat susturması bulunamadı (${targetId}). Kullanan: ${authorMember.id}`);
      ...
      const forgivenessPromises = activePunishments.map(async (punishment) => {
           try {
               ...
               return true;
                ...
                console.error(`[${commandName}] Chat susturma affetme (ID: ${punishment.id}) hatası:`, dbError);
      ...
      if (successCount > 0) {
           ...
           console.error(`[${commandName}] Hata: Chat susturmaları kaldırılırken bir hata oluştu. Hedef: ${targetId}, Kullanan: ${authorMember.id}`);
           message.react(emojis.ERROR).catch(e => { if (e.code !== 10008) console.error("Hata tepkisi eklenemedi:", e); });
      }
