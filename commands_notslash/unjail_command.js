...
module.exports = {
  name: 'unjail',
  description: 'Bir kullanıcının aktif jail cezasını kaldırır.',
  async execute(message, args, db, settings) {
    ...
    try {
      const [activePunishments] = await db.query(
        'SELECT id, user_id, ceza_tipi, sebep, channel_id, guild_id, previous_roles FROM cezalar WHERE user_id = ? AND ceza_tipi = ? AND aktif = 1 ORDER BY id DESC',
        [targetId, cezaTipi]
      );

      if (activePunishments.length === 0) {
        console.error(`[${commandName}] Hata: Aktif jail cezası bulunamadı (${targetId}). Kullanan: ${authorMember.id}`);
      ...
      const forgivenessPromises = activePunishments.map(async (punishment) => {
           try {
               ...
               await schedulePunishmentEnd(
                   ...
                   reasonForUnpunish,
                   punishment.previous_roles // Rol verisi iletiliyor
                );
               ...
               return true;
                ...
                console.error(`[${commandName}] Jail affetme (ID: ${punishment.id}) veritabanı hatası:`, dbError);
      ...
      if (successCount > 0) {
           ...
           console.error(`[${commandName}] Hata: Jail cezaları kaldırılırken bir hata oluştu (Promise.all sonrası). Hedef: ${targetId}, Kullanan: ${authorMember.id}`);
           message.react(emojis.ERROR).catch(e => { if (e.code !== 10008) console.error("Hata tepkisi eklenemedi:", e); });
      }
