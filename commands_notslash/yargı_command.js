  ...
  async execute(message, args, db, settings) {
    ...
    if (!targetId || !/^\d{17,19}$/.test(targetId)) {
      ...
      message.react(emojis.ERROR).catch(e => { if (e.code !== 10008) console.error("Hata tepkisi eklenemedi:", e); });
       try {
          const replyMsg = await message.reply('Lütfen geçerli bir kullanıcı ID\'si girin. (Kullanım: `.yargı <ID> <Sebep>`)');
          setTimeout(() => {
    ...
    if (!reasonInput) {
        ...
        message.react(emojis.ERROR).catch(e => { if (e.code !== 10008) console.error("Hata tepkisi eklenemedi:", e); });
         try {
          const replyMsg = await message.reply('Lütfen bir sebep girin. (Kullanım: `.yargı <ID> <Sebep>`)');
          setTimeout(() => {
            replyMsg.delete().catch(e => { if (e.code !== 10008) console.error("Mesaj silinemedi:", e); });
          }, 7000);
    ...
    try {
        const banReason = `Yargı: ${reason} (Yetkili: ${authorMember.user.tag} - ${authorMember.id})`;
        await message.guild.members.ban(targetId, { reason: banReason, deleteMessageSeconds: 0 }); // Mesajları silme (0 saniye)
